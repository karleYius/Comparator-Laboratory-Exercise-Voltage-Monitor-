...
void HAL_ADC_LevelOutOfWindowCallback(ADC_HandleTypeDef *hadc) {
	if(hadc->Instance == ADC1) {
		...
		watchdog_config.Channel = ADC_CHANNEL_0;
		watchdog_config.ITMode = ENABLE;

		if(state == 0) {
			HAL_GPIO_WritePin(GPIOF, GPIO_PIN_11, !GPIO_PIN_RESET);
			state = 1;
			watchdog_config.HighThreshold = 4095;
			watchdog_config.LowThreshold = 2035;
		} else {
			HAL_GPIO_WritePin(GPIOF, GPIO_PIN_11, !GPIO_PIN_SET);
			state = 0;
			watchdog_config.HighThreshold = 2048;
			watchdog_config.LowThreshold = 0;
		}

		HAL_ADC_AnalogWDGConfig(hadc, &watchdog_config);
	}
}
/* USER CODE END 0 */
